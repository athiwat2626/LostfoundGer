...
import { Search, MapPin, Clock, Tag, Filter, PackageOpen, X, CalendarDays, Layers, Palette, Building2, FileText } from "lucide-react";
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-3 text-slate-600">
              ...
              <span className="text-slate-400 w-24 shrink-0">วันที่พบ</span>
              <span className="font-medium text-slate-800">{formatDate(item.found_date)}</span>
            </div>
            <div className="flex items-center gap-3 text-slate-600">
              <MapPin size={16} className="text-slate-400 shrink-0" />
              <span className="text-slate-400 w-24 shrink-0">สถานที่พบ</span>
              <span className="font-medium text-slate-800">{item.found_location || "-"}</span>
            </div>
            ...
            <div className="flex items-center gap-3 text-slate-600">
              ...
              <span className="font-medium text-slate-800">{timeAgo(item.created_at || item.found_date)}</span>
            </div>
          </div>
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1">
          <span className="flex items-center gap-1 text-slate-400 text-xs">
            <Clock size={12} />
            {timeAgo(item.created_at || item.found_date)}
          </span>
          {item.found_location && (
            <span className="flex items-center gap-1 text-slate-400 text-xs">
              <MapPin size={12} />
        ...
        <span className={`self-start text-xs font-medium px-2.5 py-0.5 rounded-full border flex items-center gap-1 ${badgeStyle}`}>
          <Tag size={10} />
          {cat}
        </span>
...
const FoundPage = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/foundItem")
    ...
    <div className="min-h-screen bg-[#f7f3ea]">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5">
        <div className="max-w-7xl mx-auto flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900">รายการของที่พบ</h1>
            <p className="text-sm text-slate-500 mt-0.5">
              ของที่มีผู้พบและรายงานเข้าสู่ระบบ — {items.length} รายการ
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 bg-slate-100 rounded-xl px-4 py-2.5 w-full sm:w-80">
            ...
            <input
              ...
              placeholder="ค้นหาของ โทรศัพท์ กระเป๋า..."
          ...
          {!loading && filteredItems.length === 0 && (
            <div className="text-center py-24 text-slate-400">
              ...
              <p className="text-base font-medium">
                {items.length === 0 ? "ยังไม่มีรายการของที่พบ" : "ไม่พบรายการที่ตรงกัน"}
              </p>
